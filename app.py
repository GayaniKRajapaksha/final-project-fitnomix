...
# Testing flag to disable heavy operations during pytest/import
IS_TESTING = os.environ.get('APP_TESTING') == '1'
...
if not IS_TESTING:
    ...
    print("Testing mode detected: skipping model loading.")
            ...
            try:
                ...
                # Get predictions for categorical targets
                    for target_name in ['diet_recommendation', 'meal', 'ingredients']:
                        if target_name in classifiers and label_encoders and target_encoders:
                            ...
                            try:
                                ...
                                if target_name == 'meal':
                                    predictions[target_name] = get_meal_from_csv(data)
                                elif target_name == 'ingredients':
                                    predictions[target_name] = get_ingredients_from_csv(predictions.get('meal', ''))
                                else:
                    ...
                    try:
                        ...
                        # Use CSV for meal and ingredients
                        predictions['meal'] = get_meal_from_csv(data)
                        predictions['ingredients'] = get_ingredients_from_csv(predictions['meal'])
                        ...
                        predictions = {
                            ...
                            'meal': get_meal_from_csv(data),
                            'ingredients': get_ingredients_from_csv(get_meal_from_csv(data)),
...
# CSV-driven helpers for meal and ingredients

def get_meal_from_csv(data):
    """Select a meal from meals.csv based on budget when available."""
    try:
        if meals_df is None or meals_df.empty or 'meal' not in meals_df.columns:
            return ''
        budget = data.get('budget', None)
        df = meals_df.dropna(subset=['meal']).copy()
        if 'budget' not in df.columns:
            df['budget'] = np.nan
        # If budget provided, pick the most expensive meal within budget; else pick the cheapest available
        if isinstance(budget, (int, float)) and budget and budget > 0:
            within = df[df['budget'].fillna(np.inf) <= float(budget)]
            if not within.empty:
                row = within.sort_values('budget', ascending=False).iloc[0]
                return str(row['meal'])
        # Fallback: pick the meal with smallest budget (or first if budgets missing)
        try:
            row = df.sort_values('budget', ascending=True, na_position='last').iloc[0]
            return str(row['meal'])
        except Exception:
            return str(df.iloc[0]['meal'])
    except Exception:
        return ''

def get_ingredients_from_csv(meal_name):
    """Fetch ingredients string from meals.csv for a given meal if present."""
    try:
        if meals_df is None or meals_df.empty or 'meal' not in meals_df.columns:
            return ''
        matched = meals_df[meals_df['meal'].astype(str).str.strip().str.lower() == str(meal_name).strip().lower()]
        if not matched.empty and 'ingredients' in matched.columns:
            val = matched.iloc[0].get('ingredients')
